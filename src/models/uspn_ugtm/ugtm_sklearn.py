...
class eGTM(BaseEstimator, TransformerMixin):
    ...
    def refit(self, X, newmatX, newmatR):
        """Fits GTM to X using :class:`~ugtm.ugtm_classes.OptimizedGTM`.
        ...
        self.interimModel = ugtm_gtm.reinitialize(X, self.k,
                                                  self.m, self.s, newmatX,
                                                  self.random_state)

        matR = newmatR.T
        data = X
        betaInv = self.interimModel.betaInv
        regul = self.regul

        # m-step
        matG = ugtm_core.createGMatrix(matR)
        matW = ugtm_core.optimWMatrix(matR, self.interimModel.matPhiMPlusOne, matG, data, betaInv, regul)
        matY = ugtm_core.createYMatrix(matW, self.interimModel.matPhiMPlusOne)
        matD = ugtm_core.createDistanceMatrix(matY, data)
        betaInv = ugtm_core.optimBetaInv(matR, matD, self.interimModel.n_dimensions)

        self.reoptimizedModel = ugtm_gtm.reoptimize(X, self.interimModel,
                                                  self.regul,
                                                  self.niter,
                                                  matD,
                                                  matY,
